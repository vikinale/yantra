...
use System\Utilities\MediaManager;
...
class Request
{
    ...
    public function inputMedia(string $name, ?string $default = null, array $options = []): ?string
    {
        $file = $this->file($name);
        if (is_null($file) || ($file['error'] ?? 1) !== 0) return $default;
        try {
            $mediaId = MediaManager::saveUploadedFile($file, $options);
            return (string)$mediaId;
        } catch (\Throwable $e) {
            error_log('Request::inputMedia - MediaManager error: ' . $e->getMessage());
            return $default;
        }
    }
    ...
    {
        try {
            return MediaManager::getMedia($media_id);
        } catch (\Throwable $e) {
            error_log('Request::getMedia - MediaManager error: ' . $e->getMessage());
            return [];
        }
    }
