    ...
    $.event.special.destroyed || ($.event.special.destroyed = {
        ...
        maxlength: function (options, callback) {
            ...
            function showRemaining(currentInput, indicator) {
                indicator.css({display: "block"}), currentInput.trigger("maxlength.shown")
            }
            ...
            function manageRemainingVisibility(remaining, currentInput, maxLengthCurrentInput, maxLengthIndicator) {
                ...
                maxLengthIndicator && (maxLengthIndicator.html(updateMaxLengthHTML(currentInput.val(), maxLengthCurrentInput, maxLengthCurrentInput - remaining)), remaining > 0 ? (input = currentInput, threshold = options.threshold, maxlength = maxLengthCurrentInput, output = !0, !options.alwaysShow && maxlength - inputLength(input) > threshold && (output = !1), output ? showRemaining(currentInput, maxLengthIndicator.removeClass(options.limitReachedClass).addClass(options.warningClass)) : function (currentInput, indicator) {
                    options.alwaysShow || (indicator.css({display: "none"}), currentInput.trigger("maxlength.hidden"))
                }(currentInput, maxLengthIndicator)) : showRemaining(currentInput, maxLengthIndicator.removeClass(options.warningClass).addClass(options.limitReachedClass))), options.customMaxAttribute && (remaining < 0 ? currentInput.addClass("overmax") : currentInput.removeClass("overmax"))
            }
            ...
            function place(currentInput, maxLengthIndicator) {
                ...
                if ("function" !== $.type(options.placement)) if ($.isPlainObject(options.placement)) !function (placement, maxLengthIndicator) {
                ...
                }(options.placement, maxLengthIndicator); else {
                    var inputOuter = currentInput.outerWidth(), outerWidth = maxLengthIndicator.outerWidth(),
                        actualWidth = maxLengthIndicator.width(), actualHeight = maxLengthIndicator.height();
            ...
            return $.isFunction(options) && !callback && (callback = options, options = {}), options = $.extend({
                ...
                $(window).resize((function () {
                    ...
                    var maxlength = getMaxLength(currentInput), remaining = remainingChars(currentInput, maxlength),
                        output = !0;
                    ...
                    }(currentInput, maxlength), output = !1) : manageRemainingVisibility(remaining, currentInput, maxLengthCurrentInput, maxLengthIndicator), ("bottom-right-inside" === options.placement || "top-right-inside" === options.placement || "function" == typeof options.placement || options.message && "function" == typeof options.message) && place(currentInput, maxLengthIndicator), output
                }))
            }))
        }
    })
